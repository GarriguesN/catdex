...
### A.3 Web Push real — ✅ hecho
- Añadir `push` handler a `public/sw.js` + flujo de suscripción: botón en `/settings/notifications` que pida permiso y guarde la `PushSubscription` en una nueva collection `push_subscriptions` (`user`, `endpoint`, `keys`). Hecho con detección de dispositivo (`lib/push.ts`) — iOS Safari solo soporta Web Push desde la PWA instalada (Añadir a pantalla de inicio), no desde una pestaña normal; el resto (Android/desktop) se suscribe directo.
